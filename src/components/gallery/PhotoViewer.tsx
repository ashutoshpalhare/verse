...
export function PhotoViewer({ photo, onClose, onNext, onPrev, currentIndex, total }: PhotoViewerProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main Photo Area */}
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-6 overflow-hidden p-4 md:p-10 lg:flex-row lg:gap-12">
        <Button
          variant="ghost"
          size="icon"
          onClick={onPrev}
        ...
        >
          <motion.img
            ...
            src={photo.imageUrl}
            alt={photo.title}
            className="max-h-[55vh] max-w-full w-auto h-auto rounded-lg object-contain shadow-[0_0_50px_rgba(0,0,0,0.5)] md:max-h-[65vh] lg:max-h-[80vh]"
          />
        </animated.div>
        ...
        {showMetadata && (
          <motion.aside
            ...
            className="hidden w-[320px] shrink-0 self-center rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-xl lg:block"
          >
            <MetadataPanel
              photo={photo}
              align="left"
        ...
        <Button
          ...
          size="icon"
          onClick={onNext}
          className="absolute right-6 z-20 hidden h-12 w-12 rounded-full bg-white/5 text-white/70 hover:bg-white/10 hover:text-white md:flex lg:hidden xl:flex"
        >
          <ChevronRight className="h-6 w-6" />
        </Button>
      </div>

      {/* Bottom Metadata (mobile / tablet) */}
      <AnimatePresence>
        {showMetadata && (
          <motion.div
            ...
            exit={{ y: 50, opacity: 0 }}
            className="relative z-10 w-full p-6 md:p-10 lg:hidden"
          >
...
function MetadataPanel({
  ...
  if (isEditing) {
    return (
      <div className={cn("space-y-4", centered && "mx-auto max-w-lg")}>
        <Input
          value={editTitle}
          onChange={(e) => setEditTitle(e.target.value)}
          className={cn(
            "bg-white/5 border-white/10 text-2xl font-black tracking-tighter uppercase",
            centered ? "text-center" : "text-left text-xl"
          )}
        />
        <Textarea
          ...
          className={cn(
            "bg-white/5 border-white/10 text-sm text-white/60",
            centered ? "text-center" : "text-left"
          )}
        />
        <div className={cn("flex gap-2", centered ? "justify-center" : "justify-start")}>
          <Button onClick={onSave} size="sm" className="bg-white text-black font-bold uppercase tracking-widest hover:bg-white/90">
  ...
  return (
    <div className={centered ? "text-center" : "text-left"}>
      <h2
        className={cn(
          "font-sans font-black tracking-tighter text-white uppercase",
          centered ? "text-3xl md:text-5xl" : "text-2xl"
        )}
      >
        {photo.title}
      </h2>
      <p className={cn("mt-3 text-sm text-white/60", centered && "md:text-base max-w-lg mx-auto")}>
        {photo.description}
      </p>
      <div className={cn("mt-6 flex flex-wrap items-center gap-4", centered ? "justify-center" : "justify-start")}>
